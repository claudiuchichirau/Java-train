...
public class Catalog {
    ...
    @CollectionTable(name = "grade_map", joinColumns = @JoinColumn(name = "student_id"))
    @MapKeyColumn(name = "course_id")
    @Column(name = "grade_map")
    ...
    @ElementCollection(fetch = FetchType.LAZY)
    @CollectionTable(name = "professor_ratings", joinColumns = @JoinColumn(name = "catalog_id"))
    @MapKeyColumn(name = "rating")
    @Column(name = "professor_id")
    private Map<Float, UUID> professorRatings = new HashMap<>();
    ...
    public void addReview(Student student, Profesor professor, String question, int rating) {
        if (AccessControl.isStudent.test(student)) {
            Review review = new Review(student, professor, question, rating);
    ...
    public void viewReviews(User user, UUID professorId) {
        if (AccessControl.isSecretar.test(user)) {
            List<Review> reviews = studentsReviews.stream()
                    .filter(review -> review.getProfessor().getId().equals(professorId))
                    .collect(Collectors.toList());
    ...
    private void updateCourseTypesBasedOnProfessorReviews() {
        Map<UUID, List<Review>> reviewsByProfessor = studentsReviews.stream()
                .collect(Collectors.groupingBy(review -> review.getProfessor().getId()));
    ...
    private String getProfessorName(UUID professorId) {
        Course course = courses.stream()
                .filter(c -> c.getProfessor().getId().equals(professorId))
                .findFirst()
                .orElse(null);
